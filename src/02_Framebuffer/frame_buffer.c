...
int fb_init()
{
    ...
    LOG_DEBUG("g_pixel_bytes: %d, g_fb_mem_size: %d, g_line_bytes: %d", g_pixel_bytes, (int)g_fb_mem_size, g_line_bytes);
...
int draw_ascii(int x, int y, char c, unsigned int color)
{
    if (x + 8 > g_var_info.xres ||  y + 16 > g_var_info.yres) {
        LOG_DEBUG("invalid position: (%d, %d), font size: 8*16", x, y);
        return -1;
    }

    if (c < '@') {
        LOG_DEBUG("Not support this charater: %c", c);
        return -1;
    }
